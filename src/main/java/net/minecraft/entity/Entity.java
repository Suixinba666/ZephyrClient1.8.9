...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            if (this == Minecraft.getMinecraft().thePlayer) {
                stepEvent.setState(State.PRE);
                EventBus.call(stepEvent);
            }
            ...
            if (stepEvent.getStepHeight() > 0.0F && flag1 && (d3 != x || d5 != z))
            {
                ...
                if (d11 * d11 + d8 * d8 >= x * x + z * z)
                {
                    ...
                    StepEvent step = new StepEvent(false, stepHeight, 1 + y);
                    step.setState(State.POST);
                    EventBus.call(step);
                }
            }
    ...
    public void moveFlying(float strafe, float forward, float friction)
    {
        ...
        if (f >= 1.0E-4F)
        {
            ...
            if (this instanceof EntityPlayerSP) {
                MoveEntityRotationEvent moveEntityRotationEvent = EventBus.call(new MoveEntityRotationEvent(this.rotationYaw));
    ...
    public Vec3 getLookWithGivenRotation(float partialTicks, float prevRotationYaw, float prevRotationPitch, float rotationYaw, float rotationPitch)
    {
        if (partialTicks == 1.0F)
        {
    ...
    public tech.imxianyu.utils.math.vector.Vector3d getCustomPositionVector() {
        return new tech.imxianyu.utils.math.vector.Vector3d(posX, posY, posZ);
    }
