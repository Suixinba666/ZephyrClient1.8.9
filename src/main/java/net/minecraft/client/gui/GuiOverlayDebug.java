...
public class GuiOverlayDebug extends Gui {
    ...
    protected List<String> getDebugInfoLeft() {
        ...
        if (mc.gameSettings.showDebugProfilerChart) {
            ...
            list.add("[Async GL Content Loader]");
            list.add("Total: " + AsyncGLContentLoader.threadCount);

            list.add("");

            List<AsyncGLContentLoader.LoaderThread> threads = AsyncGLContentLoader.threads;
            for (int i = 0; i < threads.size(); i++) {
                String msg = "[Thread " + i + "] ";
                AsyncGLContentLoader.LoaderThread thread = threads.get(i);

                synchronized (thread.tasks) {
                    msg += "Tasks: " + thread.tasks.size();
                }
    ...
    protected List<String> getDebugInfoRight() {
        ...
        long allocatedBuffer = NativeMemory.getBufferAllocated();
        long maxBuffer = NativeMemory.getBufferMaximum();
        List<String> list = Lists.newArrayList(
                String.format("Java: %s %dbit", System.getProperty("java.version"), this.mc.isJava64bit() ? 64 : 32),
                String.format("Mem: %2d%% %03d/%03dMB", usedMem * 100L / maxMem, bytesToMb(usedMem), bytesToMb(maxMem)),
                String.format("Allocated: %2d%% %03dMB", totalMem * 100L / maxMem, bytesToMb(totalMem)),
                String.format("Native: %2d/%2dMB", bytesToMb(allocatedBuffer), bytesToMb(maxBuffer)),
                String.format("Garbage Collection: %2dMB/s", MemoryMonitor.getAllocationRateMb()),
                "",
                ...
                String.format("OGL Version: %s", GL11.glGetString(GL11.GL_VERSION)),
                String.format("LWJGL Version: %s", Sys.getVersion()),
