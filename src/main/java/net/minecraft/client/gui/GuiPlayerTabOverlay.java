...
public class GuiPlayerTabOverlay extends Gui
{
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, ScoreObjective scoreObjectiveIn)
    {
        ...
        if (list1 != null)
        {
            ...
            if (!ZephyrSettings.reduceShaders.getValue()) {
                drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1, k1 + list1.size() * this.mc.fontRendererObj.FONT_HEIGHT, RenderSystem.hexColor(0,0,0, 50));

                ShaderUtils.doRectBlurAndBloom(width * 0.5 - l1 * 0.5 - 1, k1 - 1, l1 + 2, list1.size() * this.mc.fontRendererObj.FONT_HEIGHT + 1);
            } else {
        ...
        if (!ZephyrSettings.reduceShaders.getValue()) {
            drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1, k1 + i4 * 9, RenderSystem.hexColor(0, 0, 0, 50));


            ShaderUtils.doRectBlurAndBloom(width * 0.5 - l1 * 0.5 - 1, k1 - 1, l1 + 2, i4 * 9 + 1);
        ...
        if (list2 != null)
        {
            k1 = k1 + i4 * 9 + 1;
            if (!ZephyrSettings.reduceShaders.getValue()) {
                drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1, k1 + list2.size() * this.mc.fontRendererObj.FONT_HEIGHT, RenderSystem.hexColor(0, 0, 0, 50));

                ShaderUtils.doRectBlurAndBloom(width * 0.5 - l1 * 0.5 - 1, k1 - 1, l1 + 2, list2.size() * this.mc.fontRendererObj.FONT_HEIGHT + 1);
            } else {
    ...
    private void drawScoreboardValues(ScoreObjective p_175247_1_, int p_175247_2_, String p_175247_3_, int p_175247_4_, int p_175247_5_, NetworkPlayerInfo p_175247_6_)
    {
        ...
        {
            String s1 = Formatting.YELLOW + String.valueOf(i);
