...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        GlStateManager.ortho(0.0D, mc.displayWidth / 2.0, mc.displayHeight / 2.0,
        ...
        EventBus.call(new Render2DEvent.Render2DBeforeInventoryEvent());
        ...
        GlStateManager.loadIdentity();
        GlStateManager.translate(0.0F, 0.0F, -2000.0F);
        this.mc.mcProfiler.startSection("Zephyr Render2D");
        long current = System.currentTimeMillis();

        ShaderBlur.render(Blur.blurBuffer.framebufferTexture, 1.3f);
//        ShaderBlur.render(Blur.blurBuffer.framebufferTexture, 40, 1.3f);

        Blur.blurBuffer = RenderSystem.createFrameBuffer(Blur.blurBuffer);
//        Blur.blurBuffer = RenderSystem.createDownScaledFrameBuffer(Blur.blurBuffer, 0.5);
        Blur.blurBuffer.framebufferClear();

        mc.getFramebuffer().bindFramebuffer(true);

        BloomShader.render(Bloom.bloomBuffer.framebufferTexture, 10, 1, 0.0f, 0.7f, true);
        BloomShader.render(Bloom.bloomBuffer.framebufferTexture, 5, 1, 0.0f, 0.5f, true);
        Bloom.bloomBuffer = RenderSystem.createFrameBuffer(Bloom.bloomBuffer);
        Bloom.bloomBuffer.framebufferClear();
        ...
        GlStateManager.disableAlpha();
        GlStateManager.pushMatrix();
        GlStateManager.translate(0.0F, RenderSystem.getHeight() - 70, 0.0F);
        ...
        if (this.mc.gameSettings.keyBindPlayerList.isKeyDown() && (!this.mc.isIntegratedServerRunning() || this.mc.thePlayer.sendQueue.getPlayerInfoMap().size() > 1 || scoreobjective1 != null))
        {
            ...
            this.overlayPlayerList.renderPlayerlist((int) RenderSystem.getWidth(), scoreboard, scoreobjective1);
        }
        else
        {
        ...
        if (!mc.gameSettings.showDebugInfo) {
            EventBus.call(new Render2DEvent(partialTicks, scaledresolution));
            RenderSystem.RenderingTimes.render2D = System.currentTimeMillis() - current;
        }
...
//            this.zLevel = -90.0F;
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes)
    {
        ...
        for (Score score1 : collection)
        {
            ...
            String s2 = Formatting.RED + String.valueOf(score1.getScorePoints());
