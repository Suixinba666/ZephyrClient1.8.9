...
import tech.imxianyu.gui.multiplayerdark.dialog.dialogs.ServerInfoDialogDark;
...
public class ServerBeanDark {
    ...
    public ServerBeanDark(ServerData server) {
        this.server = server;
    ...
    public void draw(double x, double y, double width, double height, double mouseX, double mouseY, ZephyrMultiPlayerUIDark inst) {
        ...
        RoundedRect.drawRound(x, offsetY, width, height, this.getRadius(), new Color(0, 0, 0));
        ...
        FontManager.segoe20.drawString(this.server.serverName, x + 50, offsetY + 18, RenderSystem.hexColor(205, 205, 205));
        FontManager.segoe16.drawString(StringUtils.removeFormattingCodes(this.server.populationInfo).isEmpty() ? "N/A" : StringUtils.removeFormattingCodes(this.server.populationInfo), x + 50, offsetY + 30, RenderSystem.hexColor(123, 123, 123));
        ...
        FontManager.pf16.drawString(String.join("\n", motd), x + 50, offsetY + 43, RenderSystem.hexColor(125, 125, 125));
...
//        RoundedRect.drawRound(x + width - 14 - statusWidth, offsetY + 20 + 4.5, statusWidth + 8, FontManager.segoe16.getHeight() + 6, 2, new Color(0, 0, 0, Math.min((int) (this.signalHoveredAlpha * 255 + 0.5), 255)));
        FontManager.segoe14.drawString(this.status, x + width - 10 - statusWidth, offsetY + 32 + 2.5, RenderSystem.hexColor(255, 255, 255, (int) (this.signalHoveredAlpha * 150)));
        ...
        if (!inst.deleteMode) {
            ...
            RenderSystem.circle(x + width - 12, offsetY + height - 11, 6, selected ? RenderSystem.hexColor(0, 160, 255) : RenderSystem.hexColor(32, 32, 32, 223));
        }
...
//        Rect.draw(x + width - 24, offsetY + 13, 18, 14, 0xff0090ff, Rect.RectType.EXPAND);
    ...
    private void checkMouse(double x, double y, double width, double height, double mouseX, double mouseY, ZephyrMultiPlayerUIDark inst) {
        double offsetY = y - this.yAnimation;
        ...
        if (hovered && Mouse.isButtonDown(0) && !mouseDown) {
            ...
            if (!inst.deleteMode) {
                ...
                if (!hoveredEdit) {
                    ...
                    inst.dialog = new ServerInfoDialogDark(server, inst.serverList.getServers().indexOf(server));
                }
    ...
    private void setServerFail(final String error) {
        this.server.pingToServer = -1L;
        this.server.serverMOTD = error;
    }

    private void pingServer(ServerData server, ZephyrMultiPlayerUIDark inst) {
        if (inst.runningTaskCount > inst.serverCountCache * 2) {
            this.setServerFail(Formatting.GRAY + "Spamming...");
            inst.pingers.submit(() -> {});
        }
        final Future<?> future = inst.timeoutExecutor.submit(() -> {
            try {
        ...
        ++inst.runningTaskCount;
        inst.pingers.submit(new Runnable() {
            @Override
            public void run() {
                try {
                    ...
                    ServerBeanDark.this.setServerFail(Formatting.RED + "Timed out");
                }
                catch (ExecutionException e2) {
                    if (e2.getCause() instanceof UnknownHostException) {
                        ServerBeanDark.this.setServerFail(Formatting.DARK_RED + "Can't resolve hostname");
                    }
                    else {
                        ServerBeanDark.this.setServerFail(Formatting.DARK_RED + "Can't connect to server.");
                    }
                }
                catch (Exception e4) {
                    ServerBeanDark.this.setServerFail(Formatting.DARK_RED + "Can't connect to server.");
    ...
    private void checkServer(ZephyrMultiPlayerUIDark inst) {
        if (!this.server.pinged)
        {
            ...
            this.pingServer(this.server, inst);
        }
