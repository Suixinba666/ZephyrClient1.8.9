...
import tech.imxianyu.gui.multiplayerdark.dialog.dialogs.ActionsDialogDark;
...
public class ZephyrMultiPlayerUIDark extends ZephyrScreen {

    private final GuiScreen parentScreen;
    public final List<ServerBeanDark> serverBeans = new ArrayList<>();
    ...
    public int serverCountCache, runningTaskCount = 0;
    ...
    public final List<TexturedButtonDark> buttons = new ArrayList<>();

    public boolean deleteMode = false;

    public Dialog dialog = null;

    public ZephyrMultiPlayerUIDark(GuiScreen parentScreen) {
    ...
    public void addServers() {
        ...
        serverCountCache = serverList.countServers();
        timeoutExecutor = Executors.newScheduledThreadPool(serverCountCache * 2 + 5);

        this.serverBeans.clear();

        for (ServerData data : serverList.getServers()) {
            this.serverBeans.add(new ServerBeanDark(data));
        }
    }
    ...
    public void initGui() {
        ...
        b.add(new Tuple<>(new ResourceLocation("Zephyr/textures/multiplayer/add.png"), () -> {
            this.dialog = new ServerInfoDialogDark();
        }));
        ...
        b.add(new Tuple<>(new ResourceLocation("Zephyr/textures/multiplayer/remove.png"), () -> {
            if (!deleteMode) {
                ...
                for (ServerBeanDark serverBean : this.serverBeans) {
                ...
                if (isAtLeastOneServerSelected) {
                    ...
                    Tuple<String, Runnable> yes = ActionsDialogDark.buildAction("Yes", () -> {
                            deleteMode = false;

                            for (ServerBeanDark serverBean : this.serverBeans.stream().filter(s -> s.selected).collect(Collectors.toList())) {
                    ...
                    Tuple<String, Runnable> no = ActionsDialogDark.buildAction("No", () -> {
                        deleteMode = false;
                    ...
                    this.dialog = new ActionsDialogDark("Do you really want to delete these servers?", "Once deleted, it cannot be recalled", Arrays.asList(yes, no));
                } else {
                    deleteMode = false;
                }
        ...
        for (Tuple<ResourceLocation, Runnable> tuple : b) {
            ...
            if (tuple.getFirst().getResourcePath().substring(tuple.getFirst().getResourcePath().lastIndexOf("/") + 1, tuple.getFirst().getResourcePath().lastIndexOf(".")).equals("back")) {
                this.buttons.add(new TexturedButtonDark(tuple.getFirst(), RenderSystem.getWidth() * 0.5 - imgSize * 1.5 - spacing, 0, imgSize, imgSize, 60, tuple.getSecond()));
                continue;
            }

            this.buttons.add(new TexturedButtonDark(tuple.getFirst(), startX, 0, imgSize, imgSize, delay, tuple.getSecond()));
    ...
    public void drawScreen(double mouseX, double mouseY) {
        ...
        BloomShader.render(behindBloomBuffer.framebufferTexture, 15, 1, 0.0f, 1f, true);

        behindBloomBuffer.framebufferClear();
        mc.getFramebuffer().bindFramebuffer(true);

        FontManager.segoe38.drawString("Zephyr", 16, 9, RenderSystem.hexColor(222, 222, 222));
        FontManager.segoe18.drawString("Server List", 82, 19, RenderSystem.hexColor(185, 185, 185));
        ...
        for (int i = 0; i < this.serverBeans.size(); i++) {
            ServerBeanDark serverBean = this.serverBeans.get(i);
        ...
        for (TexturedButtonDark button : this.buttons) {
