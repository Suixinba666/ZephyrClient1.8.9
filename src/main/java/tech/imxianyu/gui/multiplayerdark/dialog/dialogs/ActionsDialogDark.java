...
public class ActionsDialogDark extends Dialog {
    ...
    public ActionsDialogDark(String title, String content, List<Tuple<String, Runnable>> actions) {
    ...
    public void render(double mouseX, double mouseY, ZephyrMultiPlayerUIDark inst) {
        ...
        RoundedRect.drawRound(x, y, width, height, 3, new Color(22, 22, 22, intAlpha));

        titleRenderer.drawString(this.title, x + 20, y + 17, RenderSystem.hexColor(255, 255, 255, intAlpha));

        contentRenderer.drawString(String.join("\n", strings), x + 20, y + 35, RenderSystem.hexColor(255, 255, 255, Math.min(160, intAlpha)));
        ...
        for (Tuple<String, Runnable> button : this.buttons) {
            contentRenderer.drawString(button.getFirst(), buttonsX, buttonsY, RenderSystem.hexColor(255, 255, 255, Math.min(160, intAlpha)));
            ...
            if (RenderSystem.isHovered(mouseX, mouseY, buttonsX, buttonsY, contentRenderer.getStringWidth(button.getFirst()), contentRenderer.getHeight(), -5)) {
                RoundedRect.drawRound(buttonsX, buttonsY, contentRenderer.getStringWidth(button.getFirst()), contentRenderer.getHeight(), 2, -5, new Color(232, 232, 232, Math.min(30, intAlpha)));
